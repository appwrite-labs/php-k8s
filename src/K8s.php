...
class K8s
{
    ...
    {
        $instances = collect(yaml_parse($yaml, -1))->reduce(function ($classes, array $yaml) use ($cluster) {
            $kind = $yaml['kind'];
    ...
    public static function fromTemplatedYamlFile($cluster, string $path, array $replace, ?Closure $callback = null)
    {
        return static::fromYamlFile($cluster, $path, function ($content) use ($replace, $callback) {
            foreach ($replace as $search => $replacement) {
                $content = str_replace(sprintf('{%s}', $search), $replacement, $content);
            }

            return $callback instanceof \Closure ? $callback($content) : $content;
        });
    }
    ...
    public static function registerCrd(string $class, ?string $name = null): void
    {
        static::macro(
            ...
            fn($cluster = null, array $attributes = []): object => new $class($cluster, $attributes)
        );
